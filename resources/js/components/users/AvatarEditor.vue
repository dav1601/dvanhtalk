...
export default {
    ...
    methods: {
        ...
        setState(state1) {
            ...
            this.state = state1;
            this.state.cnt =
                "HELLO" + Math.floor(Math.random() * (max - min)) + min;
        },
        paint() {
            this.context.save();
            this.context.translate(0, 0);
            this.context.fillStyle =
                "rgba(" + this.color.slice(0, 4).join(",") + ")";
        ...
        loadImage(imageURL) {
            ...
            imageObj.onload = () => {
                let imageState = self.getInitialSize(
                    imageObj.width,
                    imageObj.height
                );
        ...
        getBoundedY(y, scale) {
            ...
            let heightDiff = Math.floor(
                (height - dimensions.height / scale) / 2
            );
        ...
        getCroppingRect() {
            ...
            const imageRect = this.calculatePosition(
                this.state.image,
                dim.border
            );
            return {
